...
const Depot = require("../model/DepotCryptoMoney");
...
exports.depot = asyncHandler(async (req, res)=> {
    const {pseudo, montant, reference, typeCryptoMoneyId} = req.body;

    if(Number(montant)<=0) {
        res.status(500).json({message: 'Montant incorecte'});
    }

    const depot = await Depot.create({pseudo, montant, reference,typeCryptoMoneyId });
    ...
    try {
        const depot = await Depot.findAll({
            where: {
                ...
                [Op.and]: [
                    Sequelize.where(
                        Sequelize.col("DepotCryptoMoney.createdAt"),
                        "=",
                        Sequelize.col("DepotCryptoMoney.updatedAt")
                    ),
                ],
            },
            include: {
